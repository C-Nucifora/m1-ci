...
| DBC export check (opt-in, `run-can-export-check`) | `m1-can export --check` | a committed `.dbc` export has gone stale against its `.m1dbc` source (no-op when `m1-tools.toml` has no `[dbc]` section) |
...
| `m1-can-export-check` | `m1-can export --check` | `*.m1dbc`, `*.dbc`, `m1-tools.toml` |

`m1-can-export-check` catches a `.m1dbc` edited without regenerating its
...
sync, `1` an export is stale (run `m1-can export` and commit), `2` the config
